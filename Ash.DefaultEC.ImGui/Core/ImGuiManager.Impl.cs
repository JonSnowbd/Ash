...
using Ash.Persistence.Binary;


namespace Ash.ImGuiTools
{
	public partial class ImGuiManager : GlobalManager, IFinalRenderDelegate, IDisposable
	{
		...
		void LoadSettings()
		{
			var fileDataStore = Core.Services.GetService<FileDataStore>() ?? new FileDataStore(Ash.Storage.GetStorageRoot());
		...
		/// <summary>
		/// draws the game window and deals with overriding Ash.Input when appropriate
		/// </summary>
		void DrawGameWindow()
		{
		...
		/// <summary>
		/// converts the mouse position from global window position to the game window's coordinates and overrides Ash.Input with
