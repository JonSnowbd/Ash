   ...
   public class SimpleCamera
	{
		...
		/// <summary>
		/// The size of the cameras viewport. This is useful for a bit more performance when your
		/// internal render target is smaller than the 
		/// </summary>
		...
		/// <summary>
		/// In pixels, where the origin lies for the camera. Simply put, this point in the Camera
		/// is the middle of what you set <c>Camera.Position</c> to be.
		/// </summary>
		...
		/// <summary>
		/// Takes a screen oriented vector and 'turns' it to be in line with this Camera's
		/// rotation. Useful for mouse drag logic when the camera can be rotated.
		/// </summary>
		public Vector2 ToLateral(Vector2 vec)
        {
			var co = Mathf.Cos(-Rotation);
			var si = Mathf.Sin(-Rotation);
			return new Vector2(co * vec.X - si * vec.Y, si * vec.X + co * vec.Y);
        }
