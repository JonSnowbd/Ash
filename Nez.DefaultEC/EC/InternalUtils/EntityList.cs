...
namespace Nez
{
	public class EntityList
	{
		...
		/// </summary>
		HashSet<ECEntity> _entitiesToRemove = new HashSet<ECEntity>();
		...
		/// </summary>
		Dictionary<int, FastList<ECEntity>> _entityDict = new Dictionary<int, FastList<ECEntity>>();
		...
		HashSet<ECEntity> _tempEntityList = new HashSet<ECEntity>();
		...
		public ECEntity this[int index] => _entities.Buffer[index];
		...
		public ECEntity FindEntity(string name)
		{
			for (var i = 0; i < _entities.Length; i++)
			{
		...
		public List<ECEntity> EntitiesWithTag(int tag)
		{
			var list = GetTagList(tag);

			var returnList = ListPool<ECEntity>.Obtain();
		...
		public List<T> EntitiesOfType<T>() where T : ECEntity
		{
		...
		/// <typeparam name="T">The 1st type parameter.</typeparam>
		public T FindComponentOfType<T>() where T : ECComponent
		{
			for (var i = 0; i < _entities.Length; i++)
			{
		...
		/// <typeparam name="T">The 1st type parameter.</typeparam>
		public List<T> FindComponentsOfType<T>() where T : ECComponent
		{
