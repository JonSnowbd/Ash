...
namespace Ash
{
	public class ECScene : IScene
	{
		...
		/// resources you can use Core.contentManager to load them since Ash will not ever unload them.
		/// </summary>
		...
		public static ECScene CreateWithDefaultRenderer(Color? clearColor = null)
		{
			var scene = new ECScene();
		...
		public static T CreateWithDefaultRenderer<T>(Color? clearColor = null) where T : ECScene, new()
		{
			var scene = new T();
		...
		/// <returns>The with default renderer.</returns>
		[Obsolete("use new Scene() instead")]
		public static ECScene Create(Color? clearColor = null)
		{
			var scene = new ECScene();
		...
		/// <returns>The with default renderer.</returns>
		[Obsolete("use new Scene() instead")]
		public static T Create<T>(Color? clearColor = null) where T : ECScene, new()
		{
			var scene = new T();
		...
		void UpdateResolutionScaler()
		{
			...
			Input.ResolutionOffset = _finalRenderDestinationRect.Location.ToVector2();
		...
		/// </summary>
		/// <returns></returns>
		public ECEntity CreateEntity(string name)
		{
			var entity = new ECEntity(name);
		...
		public ECEntity CreateEntity(string name, Vector2 position)
		{
			var entity = new ECEntity(name);
		...
		public ECEntity AddEntity(ECEntity entity)
		{
		...
		/// </summary>
		/// <param name="entity">The Entity to add</param>
		public T AddEntity<T>(T entity) where T : ECEntity
		{
			Insist.IsFalse(Entities.Contains(entity), "You are attempting to add the same entity to a scene twice: {0}", entity);
			Entities.Add(entity);
			entity.Scene = this;
		...
		/// <returns>The entity.</returns>
		/// <param name="name">Name.</param>
		public ECEntity FindEntity(string name) => Entities.FindEntity(name);
		...
		public List<ECEntity> FindEntitiesWithTag(int tag) => Entities.EntitiesWithTag(tag);
		...
		/// <typeparam name="T">The 1st type parameter.</typeparam>
		public List<T> EntitiesOfType<T>() where T : ECEntity => Entities.EntitiesOfType<T>();
		...
		/// <typeparam name="T">The 1st type parameter.</typeparam>
		public T FindComponentOfType<T>() where T : ECComponent => Entities.FindComponentOfType<T>();
		...
		/// <typeparam name="T">The 1st type parameter.</typeparam>
		public List<T> FindComponentsOfType<T>() where T : ECComponent => Entities.FindComponentsOfType<T>();
